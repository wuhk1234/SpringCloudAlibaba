...
import com.wuhk.service.impl.SysAdminGroupServiceImpl;
...
/**
 * @className: SysAdminGroupsController
 * @description: 系统分组 控制层
 * @author: wuhk
 * @date: 2026/3/13 0013 12:43
 * @version: 1.0
 * @company 航天信息
 **/
@RestController
...
public class SysAdminGroupsController extends CommonController{
	@Autowired
	private SysAdminGroupServiceImpl sysAdminGroupServiceImpl;
	...
	public String index(HttpServletRequest request) {
		List<Map<String, Object>> goups = sysAdminGroupServiceImpl.getDataList();
	...
	public String read(@PathVariable Integer id, HttpServletRequest request) {
		SysAdminGroup goup = sysAdminGroupServiceImpl.selectByPrimaryKey(id);
	...
	public String save(@RequestBody(required=false) SysAdminGroup record, HttpServletRequest request) {
		...
		int row = sysAdminGroupServiceImpl.save(record);
		if(row == 0) {
	...
	public String delete(@PathVariable Integer id) {
		int row = sysAdminGroupServiceImpl.deleteByPrimaryKey(id);
		if(row == 0) {
	...
	public String deletes(@RequestBody Map<String, Object> params) {
		...
		try {
			for (int i = 0; i < ids.size(); i++) {
				sysAdminGroupServiceImpl.deleteByPrimaryKey(ids.get(i));
			}
		} catch (Exception e) {
			return FastJsonUtils.resultError(-200, "保存失败", null);
		}
	...
	public String enables(@RequestBody Map<String, Object> params) {
		...
		try {
			for (int i = 0; i < ids.size(); i++) {
				...
				sysAdminGroupServiceImpl.updateByPrimaryKeySelective(record);
			}
		} catch (Exception e) {
			return FastJsonUtils.resultError(-200, "保存失败", null);
		}
