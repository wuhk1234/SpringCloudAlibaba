...
import com.wuhk.service.impl.SysAdminOrderInfoServiceImpl;
...
/**
 * @className: SysAdminOrderInfoController
 * @description: 系统订单信息控制层
 * @author: wuhk
 * @date: 2026/3/13 0013 20:00
 * @version: 1.0
 * @company 航天信息
 **/
@RestController
@RequestMapping("/admin/order")
@Api(value = "SysAdminOrderInfoController", description="系统订单接口")
public class SysAdminOrderInfoController extends CommonController{
    @Autowired
    private SysAdminOrderInfoServiceImpl sysAdminOrderInfoServiceImpl;
    ...
    @ResponseBody
    public String index(SysAdminOrderInfo record, HttpServletRequest request) {
        PageInfo<SysAdminOrderInfo> userPage = sysAdminOrderInfoServiceImpl.getDataList(record);
    ...
    public String read(@PathVariable Integer id, HttpServletRequest request) {
        SysAdminOrderInfo goup = sysAdminOrderInfoServiceImpl.selectByPrimaryKey(id);
    ...
    @ResponseBody
    public String save(@RequestBody(required=false) SysAdminOrderInfo record, HttpServletRequest request) {
        int row = sysAdminOrderInfoServiceImpl.save(record);
        if(row == 0) {
    ...
    @ResponseBody
    public String update(@RequestBody(required=false) SysAdminOrderInfo record, HttpServletRequest request) {
        int row = sysAdminOrderInfoServiceImpl.save(record);
        if(row == 0) {
    ...
    public String delete(@PathVariable Integer id) {
        int row = sysAdminOrderInfoServiceImpl.deleteByPrimaryKey(id);
        if(row == 0) {
    ...
    public String deletes(@RequestBody Map<String, Object> params) {
        ...
        try {
            for (int i = 0; i < ids.size(); i++) {
                sysAdminOrderInfoServiceImpl.deleteByPrimaryKey(ids.get(i));
            }
        } catch (Exception e) {
            return FastJsonUtils.resultError(-200, "操作失败", null);
        }
    ...
    public String enables(@RequestBody Map<String, Object> params) {
        ...
        try {
            for (int i = 0; i < ids.size(); i++) {
                SysAdminOrderInfo record = new SysAdminOrderInfo();
                record.setId(Integer.valueOf(ids.get(0)));
                record.setStatus(status);
                sysAdminOrderInfoServiceImpl.updateByPrimaryKeySelective(record);
            }
        } catch (Exception e) {
            return FastJsonUtils.resultError(-200, "保存失败", null);
        }
