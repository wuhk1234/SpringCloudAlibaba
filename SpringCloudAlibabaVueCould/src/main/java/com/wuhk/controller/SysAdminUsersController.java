...
/**
 * @className: SysAdminUsersController
 * @description: 系统用户控制层
 * @author: wuhk
 * @date: 2026/3/13 0013 18:20
 * @version: 1.0
 * @company 航天信息
 **/
@RestController
...
public class SysAdminUsersController extends CommonController{
	...
	public String index(SysAdminUser record, HttpServletRequest request) {
		PageInfo<SysAdminUser> userPage = sysAdminUserServiceImpl.getDataList(record);
	...
	public String read(@PathVariable Integer id, HttpServletRequest request) {
		SysAdminUser goup = sysAdminUserServiceImpl.selectByPrimaryKey(id);
	...
	public String save(@RequestBody(required=false) SysAdminUser record, HttpServletRequest request) {
		int row = sysAdminUserServiceImpl.save(record);
		if(row == 0) {
	...
	public String delete(@PathVariable Integer id) {
		int row = sysAdminUserServiceImpl.deleteByPrimaryKey(id);
		if(row == 0) {
	...
	public String deletes(@RequestBody Map<String, Object> params) {
		...
		try {
			for (int i = 0; i < ids.size(); i++) {
				sysAdminUserServiceImpl.deleteByPrimaryKey(ids.get(i));
			}
		} catch (Exception e) {
			return FastJsonUtils.resultError(-200, "操作失败", null);
		}
	...
	public String enables(@RequestBody Map<String, Object> params) {
		...
		try {
			for (int i = 0; i < ids.size(); i++) {
				...
				sysAdminUserServiceImpl.updateByPrimaryKeySelective(record);
			}
		} catch (Exception e) {
			return FastJsonUtils.resultError(-200, "保存失败", null);
		}
