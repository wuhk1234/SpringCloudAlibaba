...
import com.wuhk.mapper.SysAdminMenuMapper;
import com.wuhk.entity.SysAdminGroup;
import com.wuhk.entity.SysAdminMenu;
import com.wuhk.service.SysAdminMenuService;
...
public class SysAdminMenuServiceImpl extends BaseServiceImpl<SysAdminMenu> implements SysAdminMenuService {
	 ...
	 */
	@Override
	public List<SysAdminMenu> getMenusByUserId(Integer userId, Byte status) {
		...
		if(userId.equals(1)) {
			...
			//查询分组
			List<SysAdminGroup> groupsList = sysAdminGroupMapper.selectByUserId(userId, status);
			...
			//查询菜单
			menusList =  sysAdminMenuMapper.selectInRuleIds(ruleIds.toString(), 1);
		} 
     ...
     */
	@Override
	public List<SysAdminMenu> buildByRecursiveTree(List<SysAdminMenu> rootSysAdminMenus){
	 ...
	 */
	@Override
	public SysAdminMenu getChild(SysAdminMenu treeMenu, List<SysAdminMenu> treeNodes, int level) {
